...
#include<mutex>
#include <include/EPnPSolver.h>
#include <include/REPPnPSolver.h>

namespace ORB_SLAM2
{
    int Optimizer::mNullspaceEtimationItr = 20;

    Optimizer::PnPAlgorithm Optimizer::mPnPAlgorithm = Optimizer::PnPAlgorithm::LM;

    bool Optimizer::SetAlgorithm(const std::string &algorithm)
    {
        const std::map<std::string, Optimizer::PnPAlgorithm> mapping = {{"lm",     PnPAlgorithm::LM},
                                                                        {"epnp",   PnPAlgorithm::EPnP},
                                                                        {"reppnp", PnPAlgorithm::REPPnP}};

        const auto newAlgorithm = mapping.find(algorithm);
        if (newAlgorithm == mapping.end())
        {
            return false;
        }
        mPnPAlgorithm = newAlgorithm->second;
        return true;
    }
    ...
    {
        switch (mPnPAlgorithm)
        {
            case LM:
                return OptimizePoseLevenbergMarquardt(pFrame);
            case EPnP:
                return EPnPSolver::Optimize(*pFrame);
            case REPPnP:
                REPPnPSolver::Optimize(*pFrame, mNullspaceEtimationItr);
                return OptimizePoseLevenbergMarquardt(pFrame, 1, 5, true);
        }
    }
    ...
    int Optimizer::OptimizePoseLevenbergMarquardt(Frame *pFrame, int outlierRemoveItr, int lmItr, bool retainOutliers)
    {
        g2o::SparseOptimizer optimizer;
        g2o::BlockSolver_6_3::LinearSolverType *linearSolver;
        ...
        int nBad = 0;
        for (size_t it = 0; it < outlierRemoveItr; it++)
        {

            vSE3->setEstimate(Converter::toSE3Quat(pFrame->mTcw));
            optimizer.initializeOptimization(0);
            optimizer.optimize(lmItr);
