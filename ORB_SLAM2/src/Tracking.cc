...
namespace ORB_SLAM2
{
    ...
    bool Tracking::TrackWithMotionModel()
    {
        ...
        const double dur = std::chrono::duration_cast<std::chrono::duration<double> >(end - start).count();
        this->pnpTimes.emplace_back(dur);
    ...
    void Tracking::SaveFeatureMatchingTimes()
    {
        constexpr auto filename = "PnPTime.txt";
        ...
        double sum = 0.0;
        str.precision(10);
        for (const auto t : pnpTimes)
        {
            str << t << std::endl;
            sum += t;
        }
        std::cout << "PnP time ave: " << (sum / (double) pnpTimes.size()) << std::endl;
    }
