...
namespace REPPnP
{
    ...
    struct REPPnP_Options
    {
        ...
        /**
        * @brief カイ二乗分布を用いてalgebraic errorのしきい値を決定する
        * @details Default Value = false.
        */
        bool use_chi2;

        /**
         * @brief use_chi2がtrueのとき、しきい値となるχ^2の値
         * @details Default Value = 5.991
         */
        float chi2;

        /**
         * @brief use_chi2がtrueのとき、max_algebraic_error = k_per_f * sqrt(chi2 * sigma2)
         */
        float k_per_f;

        int nullspace_etimation_itr;
    };
     ...
     */
    std::tuple<TF, std::vector<idx_t>> REPPnP(Coordinates2D_t i_P, Coordinates3D_t w_P, const CameraCalibration_t K,
                                              const REPPnP_Options &opt = REPPnP_Options(),
                                              const std::vector<double> &sigma2 = std::vector<double>());
    ...
    namespace internal
    {
        ...
        */
        ...
        template <class T>
        T partition(std::vector<T> &arr, const int l, const int r)
        {
            T x = arr[r];
            int i = l;
            for (int j = l; j <= r - 1; j++) {
        ...
        // element in [l, r] using QuickSort
        // based method.  ASSUMPTION: ALL ELEMENTS
        // IN ARR[] ARE DISTINCT
        template <class T>
        T kthSmallest(std::vector<T> &arr, const int l, const int r, const int k)
        {
